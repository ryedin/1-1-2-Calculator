...
r.apps.calc = Class.create({
	initialize: function(options) { //put this here in case I later want to add constructor options
		this.currentValue = [];
		this.previousValues = [];
		this.currentOperation = r.apps.calc.operations.none;
	...
	finishPendingOperation: function() {
		if ((this.pendingValue || this.pendingValue == 0 || this.pendingValue < 0) && 
			(this.currentOperation && this.currentOperation !== r.apps.calc.operations.none && 
				this.currentOperation !== r.apps.calc.operations.equals)) {
	...
	add: function() {
		...
		if (!this.error) {
			this.currentOperation = r.apps.calc.operations.add;
			this.newline = true;
		}
	},
	subtract: function() {
		this.finishPendingOperation();
		if (!this.error) {
			this.currentOperation = r.apps.calc.operations.subtract;
			this.newline = true;
		}
	},
	multiply: function() {
		this.finishPendingOperation();
		if (!this.error) {
			this.currentOperation = r.apps.calc.operations.multiply;
			this.newline = true;
		}
	},
	divide: function() {
		this.finishPendingOperation();
		if (!this.error) {
			this.currentOperation = r.apps.calc.operations.divide;
			this.newline = true;
		}
	},
	percent: function() {
